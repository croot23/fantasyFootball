			...
			<tbody>
				<c:forEach var="team" items="${teams}">
					<tr>
						...
						<!--<td></td>-->
						<td>${team.captain.webName}
							(${team.captain.gameweekPoints*2})</td>
						<td>${team.gameweekPoints}<button
								class="mainScreenInfoButtons" type="button" data-toggle="modal"
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<div class="modal-body">
									<ul>
										<li><b>Overall Rank:</b> <span class="teamInfoValues">${team.overallRank}</span>
										</li>
										<li><b>Points On The Bench:</b> <span
											class="teamInfoValues">${team.substitutePoints}</span></li>
									</ul>
									<p>
									...
									<c:forEach var="transfer" items="${team.weeklyTransfers}">
										<ul>
											<li>${transfer.key.webName}
												(${transfer.key.gameweekPoints}) ->
												${transfer.value.webName} (${transfer.value.gameweekPoints})</li>
										</ul>
									</c:forEach>
								</div>
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<ul>
										<c:forEach var="player" items="${team.players}">
											<c:if test="${player.position == 1}">
												<li><span style="color: grey; margin-right: 5px">GK:</span>
													${player.webName} <c:choose>
														<c:when
															test="${player.webName.equals(team.captain.webName)}">
															<b>(C)</b>
															<span class="playerModalInfo">${player.gameweekPoints*2}</span>
														</c:when>
														<c:otherwise>
															<span class="playerModalInfo">${player.gameweekPoints}</span>
														</c:otherwise>
													</c:choose></li>
											</c:if>
										</c:forEach>
										<c:forEach var="player" items="${team.players}">
											<c:if test="${player.position == 2}">
												<li><span style="color: grey; margin-right: 5px">DF:</span>
													${player.webName} <c:choose>
														<c:when
															test="${player.webName.equals(team.captain.webName)}">
															<b>(C)</b>
										...
										<c:forEach var="player" items="${team.players}">
											<c:if test="${player.position == 3}">
												<li><span style="color: grey; margin-right: 5px">MF:</span>
													${player.webName} <c:choose>
														<c:when
															test="${player.webName.equals(team.captain.webName)}">
															<b>(C)</b>
										...
										<c:forEach var="player" items="${team.players}">
											<c:if test="${player.position == 4}">
												<li><span style="color: grey; margin-right: 5px">FW:</span>
													${player.webName} <c:choose>
														<c:when
															test="${player.webName.equals(team.captain.webName)}">
															<b>(C)</b>
									...
									<ul>
										<c:forEach var="substitute" items="${team.substitutes}">
											<c:if test="${substitute.position == 1}">
												<li><span style="color: grey; margin-right: 5px">GK:</span>
													${substitute.webName} <span class="playerModalInfo">${substitute.gameweekPoints}</span>
												</li>
											</c:if>
										</c:forEach>
										<c:forEach var="substitute" items="${team.substitutes}">
											<c:if test="${substitute.position == 2}">
												<li><span style="color: grey; margin-right: 5px">DF:</span>
													${substitute.webName} <span class="playerModalInfo">${substitute.gameweekPoints}</span>
												</li>
											</c:if>
										...
										<c:forEach var="substitute" items="${team.substitutes}">
											<c:if test="${substitute.position == 3}">
												<li><span style="color: grey; margin-right: 5px">MF:</span>
													${substitute.webName} <span class="playerModalInfo">${substitute.gameweekPoints}</span>
												</li>
											</c:if>
										...
										<c:forEach var="substitute" items="${team.substitutes}">
											<c:if test="${substitute.position == 4}">
												<li><span style="color: grey; margin-right: 5px">FW:</span>
													${substitute.webName} <span class="playerModalInfo">${substitute.gameweekPoints}</span>
												</li>
											</c:if>
